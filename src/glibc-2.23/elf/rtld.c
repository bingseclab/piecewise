...
static void remove_code_pages(struct link_map *map) {
  pageinfo_t *pageinfo = NULL;

  for(pageinfo=map->code_page_lst; pageinfo != NULL; pageinfo=(pageinfo->hh.next)) {
    if (pageinfo->load_fn_count == 0) {
      #if __ELF_NATIVE_CLASS == 64
        if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
          _dl_debug_printf("Unmap code page: %lx\n", pageinfo->page_num);
      #else
        if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
          _dl_debug_printf("Unmap code page: %x\n", pageinfo->page_num);
      #endif
      __mprotect((void *)pageinfo->page_num, PAGE_SIZE, PROT_NONE);
    }
  }
}

//Update pageinfo when load new function
static void update_page_info(struct link_map *map, loadinfo_t *load) {
  ElfW(Addr) page_num_start = (size_t) load->start_addr & -PAGE_SIZE;
  ElfW(Addr) page_num_end = ((size_t) load->start_addr + load->func_sz) & -PAGE_SIZE;
  pageinfo_t *code_page = NULL;
  ElfW(Addr) page_num = (ElfW(Addr)) page_num_start;

  for(; page_num <= page_num_end; page_num+=PAGE_SIZE) {
    HASH_FIND_PTR(map->code_page_lst, &page_num, code_page);
    if(code_page) {
      code_page->load_fn_count++;
    }
    #if __ELF_NATIVE_CLASS == 64
      if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
        _dl_debug_printf("load fn %lx from page %lx\n", load->start_addr, page_num);
    #else
      if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
        _dl_debug_printf("load fn %x from page %x\n", load->start_addr, page_num);
    #endif
  }
}

// Add a new dep list
static void add_dep(struct link_map *map, dep_lst_t *new_list) {
  dep_lst_t *s;
  HASH_FIND_PTR(map->dep_graph, &new_list->start_addr, s);
  if(s == NULL) {
    HASH_ADD_PTR(map->dep_graph, start_addr, new_list);
  }
}

//Load a function's dependencies using dep_graph
static void load_dep_funcs(struct link_map *map, ElfW(Addr) addr) {
  dep_lst_t *dep_lst = NULL;
  loadinfo_t *load = NULL;
  unsigned i = 0;
  HASH_FIND_PTR(map->dep_graph, &addr, dep_lst);
  if(dep_lst) {
    //Load all funcs in the list
    for(i=0; i<dep_lst->nfuncs; i++) {
      HASH_FIND_PTR(map->load_lst, &dep_lst->funcs[i]->start_addr, load);
      if(load){
        if((load->start_addr > map->l_addr) && (load->start_addr < map->l_text_end)){
          load->loaded = 1;
          update_page_info(map, load);
        }
      }else {
        ;
      }
...
// Check if address within map's executable segment
static char is_map_ro(struct link_map *map, ElfW(Addr) addr) {
  return (addr >= map->l_map_start && addr <= map->l_text_end);
}

// Wrapper for symbol lookup
static void my_lookup(void *args) {
  struct lookup_args *my_args = (struct lookup_args *) args;
  const ElfW(Sym) *ref = NULL;
  lookup_t l = _dl_lookup_symbol_x (my_args->name, my_args->map, &ref,
            my_args->map->l_scope, NULL, ELF_RTYPE_CLASS_PLT,
            DL_LOOKUP_RETURN_NEWEST, NULL);
  if(ref) my_args->result = DL_SYMBOL_ADDRESS (l, ref);
}

static void *sym_lookup(struct link_map *map, char *name) {
  const char *objname;
  const char *err_str = NULL;
  bool malloced;
...
//Get assembly function's size
static unsigned get_func_size (struct link_map *map, ElfW(Addr) start_addr) {
  if(!start_addr || map->last_sym == start_addr) return 1;
  unsigned char *addr = (unsigned char *) start_addr;
  unsigned size = 0;
  loadinfo_t *load = NULL;


  while(!load && ((ElfW(Addr))addr <= (map->last_sym))) {
    addr++;
    size++;
    HASH_FIND_PTR(map->load_lst, &addr, load);
  }
  return size;
}

// Remove unloaded functions
static void remove_func(struct link_map *map) {
  loadinfo_t *load = NULL;
  dep_lst_t *dep_list;
  for(load=map->load_lst; load != NULL; load=(load->hh.next)) {
    // Only remove functions within current object and has valid addr
    if(load->start_addr && !is_map_ro(map, load->start_addr)) continue;

    //Ignore function in unmapped pages
    if(load->pageinfo->load_fn_count == 0) {
      load->removed = 1;
      #if __ELF_NATIVE_CLASS == 64
        if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
          _dl_debug_printf("Unmapped %lx\n", load->start_addr);
      #else
        if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
          _dl_debug_printf("Unmapped %x\n", load->start_addr);
      #endif
      continue;
    }
    ...
    if(load->loaded == 0) {
      ...
      if(load->func_sz == 0) {
        HASH_FIND_PTR(map->dep_graph, &load->start_addr, dep_list);
        if(dep_list){
          load->func_sz = get_func_size(map, load->start_addr);
        }
      }

      //Prepare arguments for mprotect
      ElfW(Addr) new_start = ALIGN_DOWN (load->start_addr, GLRO(dl_pagesize));
      ElfW(Addr) new_end = ALIGN_UP (load->start_addr + load->func_sz, GLRO(dl_pagesize));

      //Remove function
      if((load->removed == 0) && __mprotect((void *) new_start, new_end - new_start, PROT_READ | PROT_WRITE) == 0){

        __builtin_memset((void *) load->start_addr, 0xd6, load->func_sz);
        __mprotect((void *) new_start, new_end - new_start, PROT_READ | PROT_EXEC);
        load->removed = 1;
        HASH_FIND_PTR(map->dep_graph, &load->start_addr, dep_list);
        #if __ELF_NATIVE_CLASS == 64
          if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
            _dl_debug_printf("Removing function %s from %s at address 0x%lx size %lu\n", dep_list->name, map->l_name, load->start_addr, load->func_sz);
        #else
          if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
          _dl_debug_printf("Removing function %s from %s\n", dep_list->name, map->l_name);
        #endif
      } else {
      }
    }
...
static void lookup_asm_func_info(struct link_map *map, dep_lst_t *lst) {
  //Lookup asm function in symbol table
  lst->start_addr = (ElfW(Addr)) sym_lookup(map, lst->name);
  if(!lst->start_addr) {
    //Try func_lst in map
    funcinfo_t *func = NULL;
    HASH_FIND_STR(map->func_lst, lst->name, func);
    if(func) {
      lst->start_addr = func->start_addr;
    }
  }
  lst->func_sz = get_func_size(map, lst->start_addr);
}

static void lookup_func_info(struct link_map *map, dep_entry_t *entry) {
  loadinfo_t *load = NULL;
  //Lookup asm function in symbol table
  entry->start_addr = (ElfW(Addr)) sym_lookup(map, entry->name);
  if(!entry->start_addr) {
    //Try func_lst in map
    funcinfo_t *func = NULL;
    HASH_FIND_STR(map->func_lst, entry->name, func);
    if(func) {
      entry->start_addr = func->start_addr;
    }
  }
  HASH_FIND_PTR(map->load_lst, &entry->start_addr, load);
  if(load) entry->size = load->func_sz;
}

// Decode .dep section
static void decode_dep(struct link_map *map)
{
  //Read .dep addr:
  size_t i;
  loadinfo_t *load = NULL;
  ...
  char *dep_saddr = (char *) map->l_map_start + map->dep_offset;
  //char *dep_eaddr = (char *) dep_saddr + map->dep_size;

  size_t lst_sz, sz_count, size;
  lst_sz = sz_count = size = 0;
  ...
  if(size) {
    map->code_ptrs = calloc(size, sizeof(ElfW(Addr)));
    for(i=0; i<size; i++) {
      map->code_ptrs[i] = *((ElfW(Addr) *) cur_addr);
  ...
  cur_addr = (char *) cur_addr + sizeof(size_t);
  while (size-- > 0) {
    //Find function size in symtab
    const ElfW(Sym) *ref = NULL;
    //Read dependency graph
    dep_lst_t *new_list = (dep_lst_t *) malloc(1*sizeof(dep_lst_t));
    //Read name
    new_list->name = cur_addr;
    cur_addr += strlen(cur_addr) + 1;
    //Read addr
    new_list->start_addr = *((ElfW(Addr) *) cur_addr);
    cur_addr = (char *) cur_addr + sizeof(void *);
    if(map->last_sym < new_list->start_addr && is_map_ro(map, new_list->start_addr)) map->last_sym = new_list->start_addr;
    //Read size
    new_list->func_sz = *((size_t *) cur_addr);
    cur_addr = (char *) cur_addr + sizeof(size_t);
    if((new_list->start_addr > map->l_text_end || new_list->start_addr < map->l_addr) && new_list->func_sz != 0) {
      new_list->start_addr = (ElfW(Addr)) sym_lookup(map, new_list->name);
    }
    if(!new_list->func_sz) {
      if(ref != NULL) new_list->func_sz = ref->st_size;
    }
    //Update size
    load = NULL;
    HASH_FIND_PTR(map->load_lst, &new_list->start_addr, load);
    if(!load) {
      load = calloc(1, sizeof(loadinfo_t));
      load->start_addr = new_list->start_addr;
      load->loaded = 0;
      load->removed = 0;
      load->func_sz = new_list->func_sz;
      HASH_ADD_PTR(map->load_lst, start_addr, load);
      
      //Add the new fn to code_page_lst
      ElfW(Addr) page_num = (size_t) load->start_addr & -PAGE_SIZE;
      HASH_FIND_PTR(map->code_page_lst, &page_num, newpage);
      if (!newpage) {
        ...
        newpage->page_map = 0;
        HASH_ADD_PTR(map->code_page_lst, page_num, newpage);
        load->pageinfo = newpage;
      } else {
        newpage->fn_count++;
        load->pageinfo = newpage;
      }
    }
    //Add this function to our own symbol table
    funcinfo_t *newfunc = NULL;
    HASH_FIND_STR(map->func_lst, new_list->name, newfunc);
    if(!newfunc) {
      newfunc = calloc(1, sizeof(funcinfo_t));
      newfunc->name = new_list->name;
      newfunc->start_addr = new_list->start_addr;
      HASH_ADD_KEYPTR(hh, map->func_lst, new_list->name, strlen(new_list->name), newfunc);
    }
    ...
    new_list->funcs = (dep_entry_t **) malloc(lst_sz*sizeof(dep_entry_t *));
    int temp;
    for(temp=0; temp<lst_sz; temp++) {
      new_list->funcs[temp] = (dep_entry_t *) malloc(1*sizeof(dep_entry_t));
      //Read name
      new_list->funcs[temp]->name = cur_addr;
      cur_addr = (char *) cur_addr + strlen(cur_addr) + 1;
      //Read dep_entry_t start address
      new_list->funcs[temp]->start_addr = *((ElfW(Addr) *) cur_addr);
      cur_addr = (char *) cur_addr + sizeof(void *);
      if(new_list->funcs[temp]->start_addr > map->last_sym && is_map_ro(map, new_list->funcs[temp]->start_addr)) 
        map->last_sym = new_list->funcs[temp]->start_addr;
      //Read dep_func size
      ...
      if(!new_list->func_sz) {
        //Find function size in symtab
        const ElfW(Sym) *ref = NULL;
        if(ref != NULL) new_list->funcs[temp]->size = ref->st_size;
      }
      load = NULL;
      HASH_FIND_PTR(map->load_lst, &new_list->funcs[temp]->start_addr, load);
      if(!load) {
        load = calloc(1, sizeof(loadinfo_t));
        load->start_addr = new_list->funcs[temp]->start_addr;
        load->loaded = 0;
        load->removed = 0;
  ...
  while(size-- > 0) {
    ...
    lookup_asm_func_info(map, new_list);
    //Read nfunc
    lst_sz = *((size_t *) cur_addr);
    //Read callees
    sz_count = 0;
    ...
    for(; sz_count < lst_sz; sz_count++) {
      ...
      cur_addr = (char *) cur_addr + strlen(cur_addr) + 1;
      lookup_func_info(map, new_list->funcs[sz_count]);
    }
    add_dep(map, new_list);
    if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
      _dl_printf("Reading dg for asm function: %s\n", new_list->name);
  }
  ...
  while(size-- > 0) {
    //Read name'
    char *name = cur_addr;
    void *func_addr = sym_lookup(map, cur_addr);
    if(!func_addr) {
      funcinfo_t *func = NULL;
      HASH_FIND_STR(map->func_lst, name, func);
      if(func) {
        func_addr = (void *) func->start_addr;
      }
    }
    HASH_FIND_PTR(map->load_lst, &func_addr, load);
    if(load) {
      load->loaded = 1;
      update_page_info(map, load);
      load_dep_funcs(map, load->start_addr);
    }
    cur_addr += strlen(cur_addr) + 1;
  }
...
// Decode content of preload section and preload libraries. Return current 
// reading index
static char *pwise_preload_libs(char *preload_section, char *section_end, struct link_map *main_map, unsigned int *npreloads) {
  size_t nlibs;
  char *cur = preload_section;
  int i=0;
  char *lib = NULL;

  //Read #libs to preload
  nlibs = *((size_t *) cur);
  cur = (char *) cur + sizeof(size_t);
  for(; i<nlibs; i++) {
    if(cur >= section_end) return NULL;
    lib = cur;
    cur += strlen(lib) + 1;
    _dl_debug_printf("Pwise preloading library %s\n", lib);
    (*npreloads) += do_preload(lib, main_map, ".preload");
  }
  if(cur >= section_end) return NULL;
  return cur;
}

static void preload_dlsym_func(char *preload_section, char *section_end, struct link_map *main_map) {
  unsigned i=main_map->l_searchlist.r_nlist;
  char *cur_addr = preload_section;
  size_t nlibs;
  size_t nfuncs;
  char *lib_name;

  nlibs = *((size_t *) cur_addr);
  cur_addr = (char *) cur_addr + sizeof(size_t);

  while(nlibs-- > 0 && cur_addr < section_end) {
    //Read library name
    lib_name = cur_addr;
    cur_addr += strlen(lib_name) + 1;

    //Read number of funcs
    nfuncs = *((size_t *) cur_addr);
    cur_addr = (char *) cur_addr + sizeof(size_t);

    //Update information in link_map
    i=main_map->l_searchlist.r_nlist;
    i--;
    while (i-- > 0) {
      struct link_map *l = main_map->l_initfini[i];
      if(strcmp(l->l_name, lib_name) == 0) {
        l->ndlsyms = nfuncs;
        l->preload_funcs = cur_addr;
      }
    }
...
static void preload_dlsym_func_doit(struct link_map *l) {
  if(!l->l_pwise) return;
  char *func_name = l->preload_funcs;
  void *func_addr = NULL;
  size_t nfuncs = l->ndlsyms;
  loadinfo_t *load = NULL;

  while(nfuncs-- > 0) {
    func_addr = sym_lookup(l, func_name);
    if(!func_addr) {
      //Try func_lst in l
      funcinfo_t *func = NULL;
      HASH_FIND_STR(l->func_lst, func_name, func);
      if(func) {
        func_addr = (void *) func->start_addr;
      }
    }
    if(func_addr) {
      HASH_FIND_PTR(l->load_lst, &func_addr, load);
      if(load) {
        load->loaded = 1;
        update_page_info(l, load);
        load_dep_funcs(l, load->start_addr);
        _dl_printf("Preload func: %s\n", func_name);
      }
    } else {
      if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE) {
        _dl_debug_printf("Failed to preload function %s in library %s\n", func_name, l->l_name);
      }
    }
...
static size_t read_preload_from_main(char *exe_name, char **preload_content) {
  ...
  int fd = __open(exe_name, O_RDONLY);
  if (fd > 0) {
    //Read header
    header = alloca(sizeof(ElfW(Ehdr)));
    if(__libc_read (fd, (void *) header, sizeof(ElfW(Ehdr))) == sizeof(ElfW(Ehdr))) {
      //Find section header
      __lseek (fd, header->e_shoff, SEEK_SET);
      shdr_len = header->e_shnum * sizeof(ElfW(Shdr));
      shdr = alloca(shdr_len);
      if ((size_t) __libc_read (fd, (void *) shdr, shdr_len) == shdr_len) {
        //Find string table
        if(header->e_shstrndx != SHN_UNDEF && header->e_shstrndx < header->e_shnum) {
          sh = shdr + header->e_shstrndx;
          if(sh->sh_size != 0) {
            strtab_len = sh->sh_size;
            strtab = alloca(strtab_len);
            __lseek (fd, sh->sh_offset, SEEK_SET);
            if ((size_t) __libc_read (fd, (void *) strtab, strtab_len) != strtab_len) {
              return 0;
            }
          }
        }
      ...
      //Find .preload section and .dep section
      if(strtab) {
        for (sh = shdr; sh < &shdr[header->e_shnum]; ++sh) {
          if(sh->sh_type == SHT_NULL) continue;
          char *sname = strtab + sh->sh_name;
          if (strcmp(sname, ".preload") == 0) {
            //Found .preload section
            _dl_debug_printf("Found .preload section in main_map\n");
            *preload_content = calloc(1, sh->sh_size);
            __lseek (fd, sh->sh_offset, SEEK_SET);
            if ((size_t) __libc_read (fd, (void *) (*preload_content), sh->sh_size) != sh->sh_size) {
              free(*preload_content);
...
#define STRING(s) __STRING (s)
  ...
  // Add entries from main_map's .preload section and add to preloads
  char *exe_name = NULL;
  for (ElfW(auxv_t) *av = GLRO(dl_auxv); av->a_type != AT_NULL; ++av) {
    if(av->a_type == AT_EXECFN) {
      exe_name = (char *) av->a_un.a_val;
    }
  }
  char *preload_content = NULL;
  size_t preload_size = 0;

  //Read .preload section
  preload_size = read_preload_from_main(exe_name, &preload_content);
  if(preload_content && preload_size)
    preload_content = pwise_preload_libs(preload_content, preload_content+preload_size, main_map, &npreloads);
...
#define VERNEEDTAG (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGIDX (DT_VERNEED))
  ...
  //Preload dlsym functions
  if(preload_content)
    preload_dlsym_func(preload_content, preload_content+preload_size, main_map);

  int load_cnt = 0;
  while (i-- > 0) {
    struct link_map *l = main_map->l_initfini[i];
    if(l->l_type != lt_library) continue;
    if(l->l_pwise) {
      if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
        _dl_debug_printf("Reading .dep section from object %s\n", l->l_name);
      decode_dep(l);
      loadinfo_t *load = NULL;
      j = main_map->l_searchlist.r_nlist;
      while(j-- > 0) {
        struct link_map *this_map = main_map->l_initfini[j];
        if(this_map->l_type != lt_library && this_map->l_type != lt_executable) continue;
        if(!this_map->import_lst) continue;
        unsigned count = 0;

        //Load imported functions
        for(; count < this_map->reloc_cnt; count++) {
          if((this_map->import_lst[count] < l->l_text_end) && (this_map->import_lst[count] > l->l_map_start)) {
            //Found a function that use pwise library code
            HASH_FIND_PTR(l->load_lst, &this_map->import_lst[count], load);
            if(load) {
              load->loaded = 1;
              update_page_info(this_map, load);
              load_cnt++;
              load_dep_funcs(l, this_map->import_lst[count]);
            }
          }
        }
      }
      ...
      unsigned count = 0;
      for(count=0; count < l->ncode_ptrs; count++) {
        if((l->code_ptrs[count] < l->l_text_end) && (l->code_ptrs[count] > l->l_map_start)) {
          HASH_FIND_PTR(l->load_lst, &l->code_ptrs[count], load);
          if(load) {
            load->loaded = 1;
            update_page_info(l, load);
            #if __ELF_NATIVE_CLASS == 64
              if(DL_DEBUG_PWISE)
                _dl_debug_printf("Load func ptr: %lx\n", load->start_addr);
            #else
              if(DL_DEBUG_PWISE)
                _dl_debug_printf("Load func ptr: %x\n", load->start_addr);
            #endif
            load_dep_funcs(l, l->code_ptrs[count]);
          }
        }
      }
      //}

      //Load initialization and termination functions (or init and fini functions)
      load = NULL;
      if (l->l_info[DT_INIT] != NULL) {
        ElfW(Addr) init_func = l->l_addr + l->l_info[DT_INIT]->d_un.d_ptr;
        HASH_FIND_PTR(l->load_lst, &init_func, load);
        if(load) {
          load->loaded = 1;
          update_page_info(l, load);
          #if __ELF_NATIVE_CLASS == 64
            if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
              _dl_debug_printf("Load init functions ptr: %lx\n", load->start_addr);
          #else
            if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
              _dl_debug_printf("Load init functions ptr: %x\n", load->start_addr);
          #endif
          load_dep_funcs(l, load->start_addr);
        }
      }
      ...
      if (init_array != NULL) {
        ...
        for (j = 0; j < jm; ++j) {
          HASH_FIND_PTR(l->load_lst, &addrs[j], load);
          if(load) {
            load->loaded = 1;
            update_page_info(l, load);
            #if __ELF_NATIVE_CLASS == 64
              if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
                _dl_debug_printf("Load init function ptr: %lx\n", load->start_addr);
            #else
              if(GLRO(dl_debug_mask) & DL_DEBUG_PWISE)
                _dl_debug_printf("Load init function ptr: %x\n", load->start_addr);
            #endif
            load_dep_funcs(l, load->start_addr);
          }
        }
      ...
      //Load functions used by dlsym
      preload_dlsym_func_doit(l);
...
#define LEN_AND_STR(str) sizeof (str) - 1, str
	...
	DL_DEBUG_SCOPES },
      { LEN_AND_STR ("pwise"), "display debloat information",
  DL_DEBUG_PWISE | DL_DEBUG_IMPCALLS },
