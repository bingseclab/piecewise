...
cannot enable executable stack as shared object requires");
  ...
  // Read .dep section and .preload section
  char *strtab = NULL;
  size_t strtab_len = 0;
  ...
  //_dl_debug_printf("Read sections from file %s\n", l->l_name);
  if(strtab) {
    for (sh = shdr; sh < &shdr[header->e_shnum]; ++sh) {
      if(sh->sh_type == SHT_NULL) continue;
      char *sname = strtab + sh->sh_name;
      //_dl_debug_printf("Found section %s\n", sname);
      //Found .dep section
      if (strcmp(sname, ".dep") == 0 && (GLRO(dl_debug_mask) & DL_DEBUG_PWISE)) {
        _dl_debug_printf("Detected pwise library %s\n", l->l_name);
        l->dep_offset = sh->sh_offset;
        l->dep_size = sh->sh_size;
        l->l_pwise = 1;
        continue;
      }
      /*
      ...
      if (strcmp(sname, ".preload") == 0) {
        //Found .preload section
        l->preload_section = calloc(1, sh->sh_size);
        if ((size_t) __libc_read (fd, (void *) l->preload_section, sh->sh_size) != sh->sh_size) {
          free(l->preload_section);
          l->preload_section = NULL;
        }
        continue;
      }
      */
