...
public:
  ...
  void addVFunc(StringRef Name) { VFuncs.push_back(Name); }
  inline unsigned getVFuncCount() { return VFuncs.size(); }
  inline StringRef getVFuncAt(unsigned i) { return VFuncs[i]; }
  void printVFuncs() {
    for(unsigned i=0; i<VFuncs.size(); i++) {
      errs() << VFuncs[i] << " ";
    }
    errs() << "\n";
  }
