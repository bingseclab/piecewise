...
void X86AsmParser::EmitInstruction(MCInst &Inst, OperandVector &Operands,
                                   MCStreamer &Out) {
  // Read callees of hand-written assembly functions
  X86Operand &Op = static_cast<X86Operand &>(*Operands[0]);
  if(Op.isToken()){
    StringRef Mnemonic = Op.getToken();
    static const char *const BranchInstrs[] = {"call", "callq", "jmp"};
    for (const char *Instr : BranchInstrs) {
      if (Mnemonic == Instr) {
        //errs() << "asm branch ";
        //Inst.dump();
        //errs() << Operands.size() << "\n";
        Op = static_cast<X86Operand &>(*Operands[1]);
        //assert(Op.isMem() && "Operand is not memory!");
        if(Op.isMem()){
          const MCExpr *Target = Op.getMemDisp();
          //sTarget->dump();
          //Write this information to .dep section
          if(Target->getKind() == MCExpr::SymbolRef){
            const MCSymbolRefExpr *SRE = cast<MCSymbolRefExpr>(Target);
            MCSymbol &Sym = const_cast<MCSymbol &>(SRE->getSymbol());
            MCSymbol *F = getParser().getCurrentFunc();
            //assert(F && "Error getting current function");
            MCSection *CurrentSection = Out.getCurrentSectionOnly();
            const MCSectionELF *ELFSection = dyn_cast_or_null<MCSectionELF>(CurrentSection);
            if(ELFSection && (ELFSection->getSectionName()==".init" 
              || ELFSection->getSectionName()==".fini")) {
              getParser().addCodePtr(&Sym);
              break;
            }else if(F) {
              //errs() << "Parsing asm func " << F->getName() << "\n";
              getParser().addCGEdge(F, &Sym);
            }
          }
        }
        break;
