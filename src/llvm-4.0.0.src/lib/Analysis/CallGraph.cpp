    ...
    for (Instruction &I : BB) {
      if (auto CS = CallSite(&I)) {
        ...
        if (!Callee || !Intrinsic::isLeaf(Callee->getIntrinsicID())) {
          ...
          // We can have direct call with Callee = nullptr. 
          //Ex: %call = call i32 bitcast (i32 (%struct._IO_FILE.1264*)* @__towrite to i32
          Value *V = CS.getCalledValue();
          if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(V)) {
            for(unsigned i=0; i<CE->getNumOperands(); i++){
              if(Function *Func = dyn_cast<Function>(CE->getOperandList()[i])) {
                if(!F->isIntrinsic())
                  Node->addOtherDirectTarget(Func);
                  // /errs() << "Add direct target " << Func->getName() << " to " << F->getName() << "\n";
                  //Node->addCalledFunction(CS, getOrInsertFunction(Func));
