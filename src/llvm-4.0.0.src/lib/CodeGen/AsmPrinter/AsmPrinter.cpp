...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  const MCExpr *CPSize = MCConstantExpr::create(CodePointers->size(), OutContext);
  OutStreamer->EmitValue(const_cast<MCExpr*>(CPSize), DL.getPointerSize());
  // Write pointers
  for(auto F : *CodePointers) {
    OutStreamer->EmitSymbolValue(OutContext.getOrCreateSymbol(F.first->getName())
          , DL.getPointerSize());
  }

  // Write dependency graph
  OutStreamer->EmitIntValue(DGI->size(), DL.getPointerSize());
  for(DepGraphInfo::iterator I=DGI->begin(); I!=DGI->end(); I++) {
    ...
    //Emit fn start addr
    OutStreamer->EmitSymbolValue(OutContext.getOrCreateSymbol(I->first->getName())
          , DL.getPointerSize());

    //Emit fn size
    if(FnSymbol != nullptr && FnSymbol->isDefined()) {
      OutStreamer->EmitValue(dyn_cast<MCSymbolELF>(FnSymbol)->getSize(), DL.getPointerSize());
    } else {
      OutStreamer->EmitZeros(DL.getPointerSize());
    }

    //Emit size current function's dep list
    const MCExpr *ListSize = MCConstantExpr::create(I->second->size(), OutContext);
    ...
    for(auto J=I->second->begin(); J!=I->second->end(); J++) {
      ...
      OutStreamer->EmitSymbolValue(OutContext.getOrCreateSymbol(CurrentFn->getName())
          , DL.getPointerSize());
      auto FnSym = OutContext.lookupSymbol(CurrentFn->getName());
      //Emit size of function
      if(FnSym != nullptr && FnSym->isDefined()) {
        OutStreamer->EmitValue(dyn_cast<MCSymbolELF>(FnSym)->getSize(), DL.getPointerSize());
      } else {
        OutStreamer->EmitZeros(DL.getPointerSize());
      }
    }
  ...
  for(DepGraphInfo::AsmCGTy::iterator I=DGI->AsmCG.begin(); I != DGI->AsmCG.end(); I++) {
  ...
  for(DepGraphInfo::InitFuncPtrTy::iterator I=DGI->InitFuncs.begin(); I != DGI->InitFuncs.end(); I++) {
