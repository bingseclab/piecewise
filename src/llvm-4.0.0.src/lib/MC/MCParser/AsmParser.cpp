...
public:
  ...
  void addCodePtr(MCSymbol *F) override { 
    FuncPtrs.push_back(F);
  }
  void printCG() override {
    for(std::map<MCSymbol *, std::vector<MCSymbol *>>::iterator I=Callgraphs.begin(); I != Callgraphs.end(); I++) {
      errs() << I->first->getName() << " calls ";
      for (auto F : I->second) errs() << F->getName() << " ";
      errs() << "\n";
    }
  }

  void printCodePtr() override {
    for(std::vector<MCSymbol *>::iterator I=FuncPtrs.begin(); I != FuncPtrs.end(); I++) {
      errs() << (*I)->getName() << " ";
    }
    errs() << "\n";
  }
...
bool AsmParser::Run(bool NoInitialTextSection, bool NoFinalize) {
  ...
  for(std::map<MCSymbol *, std::vector<MCSymbol *>>::iterator I=Callgraphs.begin(); I != Callgraphs.end(); I++) {
    ...
    Out.EmitValue(const_cast<MCExpr*>(NCallee), Ctx.getAsmInfo()->getPointerSize());
    for (auto F : I->second) {
      Out.EmitBytes(F->getName());
      Out.EmitZeros(1);
    }
