...
static void AddGoldPlugin(const ToolChain &ToolChain, const ArgList &Args,
  ...
  //Forward pwise option to gold plugin
  if (Arg *A = Args.getLastArg(options::OPT_pwise_EQ)) {
    CmdArgs.push_back(Args.MakeArgString(Twine("-plugin-opt=pwise=") + A->getValue()));
  }
  if (Arg *A = Args.getLastArg(options::OPT_pwisecpp_EQ)) {
    CmdArgs.push_back(Args.MakeArgString(Twine("-plugin-opt=pwisecpp=") + A->getValue()));
  }
}
...
// Disable the verification pass in -asserts builds.
  ...
  //Forward all pwise flags to final invocation
  Args.AddAllArgs(CmdArgs, options::OPT_pwise_EQ);
  Args.AddAllArgs(CmdArgs, options::OPT_pwisecpp_EQ);
