...
static bool ParseCodeGenArgs(CodeGenOptions &Opts, ArgList &Args, InputKind IK,
  ...
  //parse pwise options here
  if (Arg *A = Args.getLastArg(OPT_pwise_EQ)) {
    unsigned Val = llvm::StringSwitch<unsigned>(A->getValue())
                       .Case("full", CodeGenOptions::FullScan)
                       .Case("local", CodeGenOptions::LocalScan)
                       .Case("pta", CodeGenOptions::PtAnalysis)
                       .Default(~0U);

    //llvm::errs() << "CompilerInvocation read pwise mode " << static_cast<CodeGenOptions::PwiseMode>(Val) << "\n";
    if (Val == ~0U)
      Diags.Report(diag::err_drv_invalid_value) << A->getAsString(Args)
                                                << A->getValue();
    else{
      //llvm::errs() << "CompilerInvocation read pwise mode " << static_cast<CodeGenOptions::PwiseMode>(Val) << "\n";
      Opts.setPwiseInfo(static_cast<CodeGenOptions::PwiseMode>(Val));
    }
  }

  //parse pwise cpp options
  if (Arg *A = Args.getLastArg(OPT_pwisecpp_EQ)) {
    unsigned Val = llvm::StringSwitch<unsigned>(A->getValue())
                       .Case("all", CodeGenOptions::IncludeAll)
                       .Case("obj", CodeGenOptions::ObjectSensitive)
                       .Case("pta", CodeGenOptions::CppPta)
                       .Default(~0U);
    //llvm::errs() << "CompilerInvocation read pwise mode " << static_cast<CodeGenOptions::PwiseMode>(Val) << "\n";
    if (Val == ~0U)
      Diags.Report(diag::err_drv_invalid_value) << A->getAsString(Args)
      ...
      //llvm::errs() << "CompilerInvocation read pwise mode " << static_cast<CodeGenOptions::PwiseMode>(Val) << "\n";
      Opts.setPwiseCppInfo(static_cast<CodeGenOptions::PwiseCpp>(Val));
    }
  }
