...
void CodeGenFunction::GenerateCode(GlobalDecl GD, llvm::Function *Fn,
  ...
  //Add Attribute to CXXConstructors
  if(isa<CXXMethodDecl>(FD)) {
    const CXXMethodDecl *Method = cast<CXXMethodDecl>(FD);
    if(Method->isVirtual()){
      const CXXRecordDecl * Class = Method->getParent();
      unsigned cnt = 0;
      for(auto F : Class->ctors()) cnt++;
      if(cnt == 0) {
        //Include all virtual functions in this class
        Fn->addFnAttr(llvm::Attribute::GlobalDepVFunc);
      }
    }
  }
  ...
  if (isa<CXXDestructorDecl>(FD))
    ...
    const CXXConstructorDecl *C = cast<CXXConstructorDecl>(FD);
    const CXXRecordDecl *RD = C->getParent();
    Fn->addFnAttr(llvm::Attribute::CXXConstructor);
    Fn->addFnAttr(llvm::Attribute::NoInline);
    if (RD->isDynamicClass()) {
      const auto &VtableLayout = CGM.getItaniumVTableContext().getVTableLayout(RD);
      for (const auto &VtableComponent : VtableLayout.vtable_components()) {
        if (!VtableComponent.isUsedFunctionPointerKind() || !VtableComponent.isFunctionPointerKind()
          || !VtableComponent.isDestructorKind())
          continue;
        const CXXMethodDecl *Method = VtableComponent.getFunctionDecl();
        GlobalDecl GD1 = VtableComponent.getGlobalDecl();
        SmallString<256> Buffer;
        StringRef Str;
        llvm::raw_svector_ostream Out(Buffer);
        Str = CGM.getMangledName(GD1);
        Fn->addVFunc(Str);
      }
    }
